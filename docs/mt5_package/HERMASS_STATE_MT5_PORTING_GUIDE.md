...
周三，某股票：
  ...
  `state_hex(D1, W1)`：D1 close(50) > W1 阻力(49) → position = 突破
  `state_hex(W1, W1)`：W1 close(48) < W1 阻力(49) → position = 未突破
...
我们的 Python 系统同时维护 D1 Agent 和 W1 Agent（W1 close vs W1 SR），实测差异率：
...
2. **周期视角不同**——有的系统各周期独立计算 position，而非统一的 D1 视角
...
```text
Step 1: 锁定参数
  - 以本文档的数值为准（4-bit 编码、D1 视角、各窗口参数）
