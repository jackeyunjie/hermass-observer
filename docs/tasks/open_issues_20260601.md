...
| 4 | 数据同步阶段文档 `docs/2026-06-01_DATA_SYNC_PHASED_PLAN.md` | Claude | P1 | 2026-06-02 | 文档写明：同步哪些目录、排除哪些文件、失败时如何人工补救、校验 checksum 的方法；不触发实际 rsync/ssh | 待启动 |
...
| 9 | 任务模式识别修复 `mode=task` | — | P2 | TBD | `"mode":"task"` 请求时 `_chat_answer` 返回 `mode_used="task"` 而非 `"chat"`；不破坏现有 chat 路径 | **已发 KIMI 定位提示词，待分析报告** |
| 10 | 连续对话记忆稳定性 Test 7 | Claude | P2 | TBD | 第 1 轮输入含股票代码后，第 2 轮 `"它是什么行业"` 返回含该股票所属行业（电子）的分析；session_id 一致 | **已发 KIMI 提示词，待重跑验证** |
...
### Codex（待启动 4 项）
- [x] #2 Ruff/lint 脚本 — scripts/lint.sh 已交付，check/format/help 三模式，退出码 0/9/2
- [x] #3 pyproject.toml ruff 配置 — 已追加 [tool.ruff]，line-length=110，select=E/F/I
- [x] #6 部署/回滚脚本 — scripts/deploy.sh + scripts/rollback.sh 已交付；含环境检查（systemctl 不存在则拒绝）；退出码 0/3/4/5/6/7 和 0/7/8；均支持 --help

### Claude（待验证 1 项）
- [x] #4 数据同步阶段文档 — docs/2026-06-01_DATA_SYNC_PHASED_PLAN.md 已交付
- [x] #10 连续对话记忆 — 已修改 `_build_memory_context` + `chat_query` 回填逻辑；KIMI 提示词已发出（`docs/tasks/kimi_prompt_test7_rerun.md`），待重跑结果

### 未分配（定位中 1 项）
- [x] #9 任务模式识别 — KIMI 定位提示词已发出（`docs/tasks/kimi_prompt_test9_mode_task.md`），待根因报告
