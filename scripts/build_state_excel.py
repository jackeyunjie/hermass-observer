...
def _load_3day_state():
    ...
    for r in rows:
        ds, code = str(r[0]), r[1]
        out[ds][code] = {
            "close": r[2], "hex": (r[3], r[4], r[5]), "sc": (r[6], r[7], r[8]), "ef": r[9],
            "adx": r[10], "atr": r[11],
...
def _load_moneyflow():
    ...
    for code, d, an, bo, r in rows:
        ...
        out[c6][str(d)] = {"an": round(an/1e8,2) if an else 0, "bo": round(bo/1e8,2) if bo else 0, "rt": round(r*100,1) if r else 0}
    return {"dates": dates, "data": dict(out)}


def _hex_fill(hex_val) -> PatternFill:
    h = str(hex_val)
    if h in ("E", "F"): return PatternFill("solid", fgColor="CCE5FF")
    if h.startswith("-"): return PatternFill("solid", fgColor="F8D7DA")
    try:
        s = int(h, 16)
        if s >= 8: return PatternFill("solid", fgColor="D4EDDA")
...
def generate():
    s3 = _load_3day_state(); dates = s3["dates"]; sd = s3["data"]
    latest = dates[0]; codes = sorted(sd.get(latest, {}).keys())
    mf = _load_moneyflow(); mfd = mf.get("dates", []); mfv = mf.get("data", {})

    nm, sw = {}, {}
    fdb = ROOT / "outputs/fundamental/fundamental_evidence.duckdb"
    if fdb.exists():
        c = duckdb.connect(str(fdb), read_only=True)
        for code, name, sw1 in c.execute("SELECT stock_code,stock_name,sw_l1 FROM ifind_industry_chain_profile").fetchall():
            nm[code] = name or ""; sw[code] = sw1 or ""
        c.close()

    wb = openpyxl.Workbook(); ws = wb.active; ws.title = f"State_{latest}"
    ...
    headers = [
        "代码", "名称", "行业", "日期", "收盘",
        "MN1", "W1", "D1", "ef",
        f"主力净({T})", f"大单({T})", f"主力%({T})",
        f"主力净({Y})", f"大单({Y})", f"主力%({Y})",
        f"主力净({P})", f"大单({P})", f"主力%({P})",
        "ADX", "ATR%",
        "MN1支", "MN1阻", "W1支", "W1阻", "D1支", "D1阻",
    ]
    ...
    for ci, h in enumerate(headers, 1):
        ...
        cell.font = hf; cell.fill = fill_hdr; cell.alignment = cen

    row = 2
    for code in codes:
        nc = nm.get(code, ""); sc = sw.get(code, "")
        for di, d in enumerate(dates):
            ...
            vals = [code, nc, sc, day_label, round(s["close"], 2),
                    s["hex"][0], s["hex"][1], s["hex"][2], s["ef"]]
            for mdd in mfd:
                m = mfr.get(mdd, {})
            ...
            for ci, v in enumerate(vals, 1):
                ...
                cell.alignment = cen; cell.border = bdr
...
if __name__ == "__main__":
    import json
    p = generate()
    print(json.dumps({"status": "ok", "path": str(p), "size_kb": round(p.stat().st_size / 1024, 1)}, ensure_ascii=False))
