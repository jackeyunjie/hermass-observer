...
def calculate_all_states(
    ...
    for item in aligned:
        ...
        # D1 SR
        d1_sr = calculate_sr(
            d1_data.highs[:d1_idx+1],
            d1_data.lows[:d1_idx+1],
            d1_data.closes[:d1_idx+1]
        )
        
        # W1 SR (using W1 data up to current W1 bar)
        w1_sr = calculate_sr(
            w1_data.highs[:w1_idx+1],
            w1_data.lows[:w1_idx+1],
            w1_data.closes[:w1_idx+1]
        )
        
        # MN1 SR (using MN1 data up to current MN1 bar)
        mn1_sr = calculate_sr(
            mn1_data.highs[:mn1_idx+1],
            mn1_data.lows[:mn1_idx+1],
            mn1_data.closes[:mn1_idx+1]
        )
