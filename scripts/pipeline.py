...
def run_pipeline(date_str: str, data_source: str = "fixture"):
    ...
    # JSON
    json_path = (
        Path(__file__).parent.parent / "fixtures" / f"observation_pool_{date_str.replace('-', '')}.json"
    )
    output_data = {
        "date": date_str,
        ...
        "strong": strong,
        "stocks": [
            {"code": r.stock_code, "name": r.stock_name, "ef_count": r.ef_count, "states": r.states}
            for r in screened
        ],
    }
    ...
    return {"csv": csv_path, "html": html_path, "json": json_path, "screened": screened}
