...
def generate_html(rows, date_str, total_matches, require_mn1_ef=False):
    ...
    ultra_count = len({(row["股票代码"], row["股票简称"]) for row in rows if str(row.get("EF周期数")) == "3"})
    strong_count = len(
        {(row["股票代码"], row["股票简称"]) for row in rows if str(row.get("EF周期数")) == "2"}
    )
...
"""
    ...
    header_colors = {
        ...
        "日期": "col-core",
        "EF周期数": "col-core",
        "MN1_state_hex": "col-mn1",
        "MN1_state_score": "col-mn1",
        "W1_state_hex": "col-w1",
        "W1_state_score": "col-w1",
        "D1_state_hex": "col-d1",
        "D1_state_score": "col-d1",
    }

    for col in FIXED_COLUMNS:
        color_class = header_colors.get(col, "col-detail")
        display = (
            col.replace("_state_hex", "")
            .replace("_state_score", "_score")
            .replace("_label", "")
            .replace("_bit", "")
...
def main():
    ...
    print(
        f"  字段顺序: 已固定（股票代码/简称/日期 → MN1 hex/score → W1 hex/score → D1 hex/score → 详细字段）"
    )
