...
def backfill_post_suspension_days(conn: duckdb.DuckDBPyConnection) -> dict:
    """检测停牌-复牌事件，回填 post_suspension_days。

    算法：
      1. 构建市场交易日历（daily_bars 中全部 DISTINCT date）
      2. 对每只股票，找出「交易日但该股无数据」的停牌日集合
      3. 把连续停牌日聚合为一次停牌事件，记录 (stock_code, suspend_start, suspend_end, duration)
      4. 对 d1_perspective_state 中每行，计算 post_suspension_days:
         - 如果该日处于停牌中 → post_suspension_days = 停牌已持续天数
         - 如果该日处于 long-suspension 复牌后 5 日内 → post_suspension_days = 复牌后第 N 天
         - 否则 → 0
    """
    t0 = time.time()
    ...
    # 用 pandas groupby 构建每只股票的 date 集合
    ...
    # ── 4c. 检测停牌事件 ──
    # 对每只股票，找出交易日历中缺失的日期（停牌日）
    # 然后聚合连续停牌为事件
    suspension_events: list[tuple[str, int, int]] = []  # (stock_code, suspend_days_count, resume_idx)

    # 为效率，构建全局结果 DataFrame
    # stock_code, state_date → post_suspension_days
    ...
    # ── 4d. 写入 post_suspension_days ──
    if result_records:
        psd_df = pd.DataFrame(result_records)
        # 只保留列需要的字段
        psd_df = psd_df[["stock_code", "state_date", "post_suspension_days"]]

        # 创建临时表用于 JOIN UPDATE
        conn.execute("DROP TABLE IF EXISTS _tmp_post_suspension")
        conn.execute("""
            CREATE TEMPORARY TABLE _tmp_post_suspension AS
            SELECT * FROM psd_df
        """)
    ...
    nonzero = conn.execute(
        f"SELECT COUNT(*) FROM {TARGET_TABLE} WHERE post_suspension_days > 0"
    ).fetchone()[0]
    ...
    stats = {
        ...
        "rows_with_post_suspension": nonzero,
    }
...
# ── 主流程 ────────────────────────────────────────────────────
def run(foundation_db: Path) -> dict:
    t_start = time.time()
    log.info("=" * 60)
    ...
    psd_stats = backfill_post_suspension_days(conn)
