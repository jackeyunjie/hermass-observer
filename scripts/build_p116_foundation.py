...
RAW_TABLE = "blackwolf_ashare_daily_raw"
...
def build(raw_db: Path, out_db: Path, date: str) -> dict:
    ...
    conn.execute(
        ...
        SELECT stock_code, date::DATE AS date, open, high, low, close, volume, amount
        FROM rawdb.{RAW_TABLE}
        WHERE research_only_flag = true
          AND date <= DATE '{date}'
    ...
    # SqFractal 5, confirmed 3 bars later, then forward-filled.
    conn.execute(
        """
          ...
          SELECT
            *,
            lag(center_fractal_resistance, 3) OVER (
              PARTITION BY stock_code, timeframe ORDER BY period_start
    ...
    conn.execute(
        ...
        smoothed AS (
          ...
          FROM directional
          WINDOW w14 AS (
            PARTITION BY stock_code, timeframe ORDER BY period_start ROWS BETWEEN 13 PRECEDING AND CURRENT ROW
          )
        ),
        di AS (
          SELECT
    ...
    conn.execute(
        ...
        SELECT
          b.stock_code,
          b.date AS state_date,
          b.close AS d1_close,
          s.period_start AS d1_period_start,
          s.sr_support AS d1_sr_support,
          s.sr_resistance AS d1_sr_resistance,
          s.sr_ready AS d1_sr_ready
        FROM (
        ...
        ASOF LEFT JOIN (
          SELECT stock_code, available_date, period_start, sr_support, sr_resistance, sr_ready
          FROM sr_levels
          WHERE timeframe = 'D1'
          ORDER BY stock_code, available_date
        ) s
        ON b.stock_code = s.stock_code AND b.date >= s.available_date
        ORDER BY b.stock_code, b.date
        """
    )
    ...
    conn.execute(
        ...
        SELECT
          b.stock_code,
          b.date AS state_date,
          s.period_start AS w1_period_start,
    ...
    conn.execute(
        f"""
        CREATE TABLE d1_mn1_sr AS
        SELECT
          b.stock_code,
          b.date AS state_date,
          s.period_start AS mn1_period_start,
    ...
    conn.execute(
        ...
        SELECT
          ...
          m.mn1_sr_ready
        FROM d1_d_sr d
        LEFT JOIN d1_w_sr w USING (stock_code, state_date)
        LEFT JOIN d1_mn1_sr m USING (stock_code, state_date)
        ORDER BY d.stock_code, d.state_date
        """
    )
    ...
    conn.execute(
        ...
        WITH c AS (
          SELECT
            ctx.*,
            id.trend AS d1_trend,
            id.volatility AS d1_volatility,
            id.compression AS d1_compression,
            id.adx14 AS d1_adx14,
            id.plus_di_14 AS d1_plus_di_14,
            id.minus_di_14 AS d1_minus_di_14,
            id.adx_slope_3 AS d1_adx_slope_3,
            id.bb_width_pct AS d1_bb_width_pct,
            id.bb_width_q20_20 AS d1_bb_width_q20_20,
            id.bb_width_q80_20 AS d1_bb_width_q80_20,
            id.atr_ratio_pct AS d1_atr_ratio_pct,
            id.atr_ratio_avg60 AS d1_atr_ratio_avg60,
            iw.trend AS w1_trend,
            iw.volatility AS w1_volatility,
            iw.compression AS w1_compression,
            iw.adx14 AS w1_adx14,
            iw.plus_di_14 AS w1_plus_di_14,
            iw.minus_di_14 AS w1_minus_di_14,
            iw.adx_slope_3 AS w1_adx_slope_3,
            iw.bb_width_pct AS w1_bb_width_pct,
            iw.bb_width_q20_20 AS w1_bb_width_q20_20,
            iw.bb_width_q80_20 AS w1_bb_width_q80_20,
            iw.atr_ratio_pct AS w1_atr_ratio_pct,
            iw.atr_ratio_avg60 AS w1_atr_ratio_avg60,
            im.trend AS mn1_trend,
            im.volatility AS mn1_volatility,
            im.compression AS mn1_compression,
            im.adx14 AS mn1_adx14,
            im.plus_di_14 AS mn1_plus_di_14,
            im.minus_di_14 AS mn1_minus_di_14,
            im.adx_slope_3 AS mn1_adx_slope_3,
            im.bb_width_pct AS mn1_bb_width_pct,
            im.bb_width_q20_20 AS mn1_bb_width_q20_20,
    ...
    conn.execute(
        ...
        SELECT
          'p116_foundation_v0_2_mt4like' AS schema_version,
          ? AS generated_at,
          ? AS source_raw_db,
          ? AS output_duckdb,
          (SELECT count(*) FROM daily_bars) AS daily_rows,
          (SELECT count(*) FROM weekly_bars) AS weekly_rows,
          (SELECT count(*) FROM monthly_bars) AS monthly_rows,
          (SELECT count(*) FROM sr_levels) AS sr_rows,
          (SELECT count(*) FROM d1_perspective_state) AS state_rows,
          (SELECT max(state_date) FROM d1_perspective_state) AS latest_date,
          true AS research_only_flag
        """,
        [datetime.now(timezone.utc).isoformat(timespec="seconds"), str(raw_db), str(out_db)],
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build P116 reusable data foundation.")
