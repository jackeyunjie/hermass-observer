...
# ── HTML ──
def _c(v): return "#d4edda" if v>0.02 else ("#fff3cd" if v>0 else "#f8d7da")
def _b(lvl):
    m={"高":"#28a745","中":"#ffc107","中低":"#fd7e14","低":"#dc3545","噪声":"#6c757d","最优":"#0d6efd"}
...
def build_html():
    ...
    rows.append(f'全市场 <b>{total:,}</b> 只股票中，<b>{ef2:,}</b> 只处于 E/F≥2 强势状态（占比 <b>{ef2_pct}%</b>），' 
    ...
    try:
        ...
        if dbs:
            ...
            idx = dict(fcon.execute("SELECT stock_code, sw_l1 FROM ifind_industry_chain_profile WHERE sw_l1 IS NOT NULL").fetchall())
            fcon.close()
            ai_set = {"电子","计算机","通信"}; div_set = {"银行","煤炭","公用事业","交通运输","钢铁"}
            ai_codes = [c for c,i in idx.items() if i in ai_set][:3000]
            div_codes = [c for c,i in idx.items() if i in div_set][:3000]
            ai_f = "('"+"','".join(ai_codes)+"')"; div_f = "('"+"','".join(div_codes)+"')"
            ai_ef2 = con.execute(f"SELECT SUM(CASE WHEN ef_count>=2 THEN 1 ELSE 0 END) FROM d1_perspective_state WHERE state_date=CAST('{latest}' AS DATE) AND stock_code IN {ai_f}").fetchone()[0]
            div_ef2 = con.execute(f"SELECT SUM(CASE WHEN ef_count>=2 THEN 1 ELSE 0 END) FROM d1_perspective_state WHERE state_date=CAST('{latest}' AS DATE) AND stock_code IN {div_f}").fetchone()[0]
            ai_ef2_pct = round(ai_ef2/max(len(ai_codes),1)*100,1)
            div_ef2_pct = round(div_ef2/max(len(div_codes),1)*100,1)
            rows.append(f'AI叙事方向（电子+计算机+通信）强势占比 <b>{ai_ef2_pct}%</b>，'
    ...
    rows.append(f'<div class="metric"><div class="val">{ef2:,}</div><div class="lbl">E/F≥2 ({ef2_pct}%)</div></div>')
    rows.append(f'<div class="metric"><div class="val">{avg_d1}</div><div class="lbl">平均 D1 Score</div></div>')
    rows.append('</div>')
    rows.append('<table style="margin-top:10px"><tr><th>ef=0</th><th>ef=1</th><th>ef=2</th><th>ef=3</th></tr><tr>')
    for k in ["0","1","2","3"]: rows.append(f'<td>{ef_dist.get(k,0):,}</td>')
    ...
    try:
        ...
        if dbs2:
            ...
            if res:
                rows.append('<div class="card"><h2>🔥 板块共振 Top 6</h2><table><tr><th>行业</th><th>共振数</th><th>置信</th></tr>')
                for r in res[:6]:
                    rows.append(f'<tr><td><b>{r["sw_l1"]}</b></td><td>{r["resonance_count"]}只</td><td>{_b(r["confidence"])}</td></tr>')
    ...
    if calib:
        rows.append('<div class="card"><h2>🧠 MN1 分层校准</h2><table><tr><th></th><th>牛市E/F</th><th>震荡C/D</th><th>扩张8-B</th><th>破位</th></tr>')
        for sid in ["vcp","ma2560","bollinger_bandit"]:
            sd = calib.get("stratified_results",{}).get(sid,{})
            cels=[]
            for r in ["牛市环境_E/F","震荡偏强_C/D","扩张未突破_8-B","破位环境"]:
                rd=sd.get(r,{}); me=rd.get("weighted_mean_excess",0)
...
def send_email(html, smtp, to_addrs, attach):
    ...
    msg["From"] = smtp["user"]; msg["To"] = ", ".join(to_addrs)
    ...
    if attach and attach.exists():
        with open(attach,"rb") as f:
            ...
            p.add_header("Content-Disposition",f'attachment; filename="{attach.name}"'); msg.attach(p)
    with smtplib.SMTP(smtp["host"],smtp["port"],timeout=30) as s:
        s.starttls(); s.login(smtp["user"],smtp["password"]); s.sendmail(smtp["user"],to_addrs,msg.as_string())
    return True

def main():
    p = argparse.ArgumentParser(); p.add_argument("--dry", action="store_true"); args = p.parse_args()
    html = build_html()
    if args.dry:
        (ROOT/"outputs/daily_report.html").write_text(html,encoding="utf-8")
        print("HTML 已保存: outputs/daily_report.html"); return
    excel = build_excel()
    smtp = {
    ...
    if not smtp["user"] or not to: print("缺少 SMTP 凭据，检查 .envrc"); sys.exit(1)
    send_email(html, smtp, to, excel)
    print(json.dumps({"status":"ok","to":to,"excel":str(excel),"generated_at":datetime.now(timezone.utc).isoformat()},ensure_ascii=False))
