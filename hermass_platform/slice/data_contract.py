...
def validate_state_hex(value: Any, field: str) -> ContractViolation | None:
    ...
    if not _STATE_HEX_RE.match(value):
        return ContractViolation(
            field, value, "STATE_BASE_CONTRACT §3.5", f"{field}={value} 不符合 hex 格式 /^-?[0-9A-F]$/"
        )
    return None
...
def validate_ef_count(value: Any, field: str) -> ContractViolation | None:
    ...
    if value < 0 or value > 3:
        return ContractViolation(
            field, value, "STATE_BASE_CONTRACT §2.2.8", f"{field}={value} 超出 [0, 3] 范围"
        )
    return None
...
def validate_state_score(value: Any, field: str) -> ContractViolation | None:
    ...
    if value < -15 or value > 15:
        return ContractViolation(
            field, value, "STATE_BASE_CONTRACT §3.1", f"{field}={value} 超出 [-15, 15] 范围"
        )
    return None
...
def validate_slice_envelope(result: dict) -> ValidationResult:
    ...
    if ct != "1.0.0":
        vr.add_violation("contract_version", ct, "contract_v1.json", f"合同版本应为 1.0.0，实际为 {ct}")

    vt = result.get("slice_type")
    valid_types = {"user", "strategy", "time", "industry", "cognitive"}
    if vt not in valid_types:
        vr.add_violation("slice_type", vt, "contract_v1.json", f"无效切片类型: {vt}")
...
def validate_state_row(row: dict, slice_type: str) -> list[ContractViolation]:
    ...
    if not state_date or not isinstance(state_date, str):
        violations.append(
            ContractViolation(
                "state_date", state_date, "STATE_BASE_CONTRACT §2.2.8", f"state_date={state_date} 无效或缺失"
            )
        )
    ...
    if "d1_close" in row:
        ...
        if isinstance(close, (int, float)) and close <= 0:
            violations.append(
                ContractViolation(
                    "d1_close", close, "STATE_BASE_CONTRACT §5.1", f"d1_close={close} 必须为正值"
                )
            )
    ...
    if "ef_count" in row and all(f"{f}_state_score" in row for f in ("mn1", "w1", "d1")):
        ef = row["ef_count"]
        expected = sum(1 for f in ("mn1", "w1", "d1") if row.get(f"{f}_state_score") in (14, 15))
        if ef != expected:
            violations.append(
                ContractViolation(
                    "ef_count",
                    ef,
                    "STATE_BASE_CONTRACT §2.2.8",
...
def validate_slice_data(data: list, slice_type: str) -> ValidationResult:
    ...
    for i, row in enumerate(data):
        if not isinstance(row, dict):
            vr.add_violation(f"data[{i}]", type(row).__name__, "contract_v1.json", f"data[{i}] 不是 dict")
            continue
...
def compute_cache_key(slice_type: str, params: dict, cache_date: str) -> str:
    raw = json.dumps(
        {"slice_type": slice_type, "params": params, "cache_date": cache_date},
        ensure_ascii=False,
        sort_keys=True,
    )
